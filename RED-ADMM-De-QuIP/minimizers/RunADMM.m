...
function [im_out, psnr_out, ssim_out, data] = RunADMM(y, ForwardFunc, BackwardFunc,...
    InitEstFunc, input_sigma, params, orig_im, input_im_bicubic)
...
% parameters
lambda = 35* params.lambda;  % 30-40
beta = 2* params.beta;   % 2 - 2.5
outer_iters = 30; % params.outer_iters;
inner_iters = params.inner_iters;
inner_denoiser_iters = params.inner_denoiser_iters;
effective_sigma = 2.2*input_sigma;% params.effective_sigma; 1.5 - 3
...
if isfield(params,'use_fft') && params.use_fft == true
    ...
    fft_HtH = abs(fft_psf).^2 / (input_sigma^2);
                    ...
                    Nb = dr*dc;
                    [nr,nc] = size(y);
                    m = nr*nc; nrup = nr*dr; ncup = nc*dc;
                    STy = zeros(nrup,ncup);
                    STy(1:dr:end,1:dc:end)=y;
end
...
%             
                    ...
                    [FB,FBC,F2B,input_im] = HXconv(orig_im,params.psf);
                    FR = FBC.*fft2(STy) + beta*fft2(v_est - u_est);
                    x_est = INVLS(FB,FBC,F2B,FR,beta,Nb,nr,nc,m);
%                     x_est = max( min(x_est, 255), 0);
%         end
    end
    ...
    % using De-QuIP denoiser
    for j = 1:1:inner_denoiser_iters
         f_v_est = DeQuIP_Denoiser_function(v_est, effective_sigma);
    ...
    if ~QUIET && (mod(k,1) == 0 || k == outer_iters)
...
%             lambda, effective_sigma);
        ...
        ssim_out = ssim_index(im_out, orig_im, [0.01 0.03], fspecial('gaussian', 3, 1.5), max(orig_im(:)));
        fprintf('%7i %12.5f %12.5f \n', k, psnr_out, ssim_out);
        data.psnr(k) = psnr_out;
        data.ssim(k) = ssim_out;
    end
end
